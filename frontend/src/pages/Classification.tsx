  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex border-b border-gray-200 bg-white px-4 shrink-0">
        {(['golden-sets', 'prompts', 'eval-runs'] as Tab[]).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === 'golden-sets' ? 'Golden Sets' : tab === 'prompts' ? 'Prompts' : 'Eval Runs'}
          </button>
        ))}
      </div>
    ...
    <div className="flex h-full">
      <div className="flex-1 p-4 overflow-auto">
        <div className="flex items-center gap-2 mb-3">
          <button onClick={handleCreate} className="bg-blue-600 text-white px-3 py-1 rounded text-sm">New Entry</button>
          <button onClick={() => setShowImport(!showImport)} className="bg-gray-100 text-gray-700 px-3 py-1 rounded text-sm">Import</button>
        </div>

        {showImport && (
          <div className="mb-4 border border-gray-200 rounded p-3">
            <JsonEditor value={importJson} onChange={setImportJson} height="150px" />
            <button onClick={handleImport} className="mt-2 bg-blue-600 text-white px-3 py-1 rounded text-xs">Import</button>
          </div>
        )}
      ...
      {selected && (
        <div className="w-[420px] border-l border-gray-200 p-4 overflow-auto bg-gray-50">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-gray-500">Input Transactions</label>
            ...
            <div>
              <label className="text-xs font-medium text-gray-500">Expected Output</label>
              <pre className="text-xs bg-white rounded p-2 mt-1 max-h-40 overflow-auto border">
            ...
            {selected.reference_transactions != null && (
              <div>
                <label className="text-xs font-medium text-gray-500">Reference Transactions</label>
                <pre className="text-xs bg-white rounded p-2 mt-1 max-h-40 overflow-auto border">
        ...
        <div className="space-y-1">
          {prompts.map((p) => (
            <div
              ...
              className={`px-3 py-2 rounded text-sm cursor-pointer ${selected?.id === p.id ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-50'}`}
            >
              {p.name}
            </div>
          ))}
        ...
        <div className="flex-1 p-4 overflow-auto">
          <div className="space-y-4 max-w-3xl">
            <div>
              ...
              <input value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-1.5 text-sm" />
            </div>
            <div>
            ...
            <button onClick={handleSave} className="bg-blue-600 text-white px-4 py-2 rounded text-sm">Save</button>
          </div>
        </div>
      ) : (
        <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">Select or create a prompt</div>
      )}
    </div>
  )
}
      ...
      <div className="mb-6 bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="flex items-end gap-3">
          <div>
            ...
            <select value={selectedPromptId} onChange={(e) => setSelectedPromptId(e.target.value)} className="border border-gray-300 rounded px-2 py-1.5 text-sm">
              <option value="">Select...</option>
              {prompts.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Golden Set</label>
            <select value={selectedSetName} onChange={(e) => setSelectedSetName(e.target.value)} className="border border-gray-300 rounded px-2 py-1.5 text-sm">
              <option value="">Select...</option>
        ...
        {selectedRun && (
          <div>
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-gray-500">{selectedRun.golden_set_name}</span>
            </div>
            {selectedRun.metrics && (
              <MetricsCard
            ...
            {selectedRun.metrics?.per_category && (
              <div className="mt-3">
                <h4 className="text-xs font-medium text-gray-500 mb-2">Per-Category</h4>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(selectedRun.metrics.per_category).map(([cat, m]) => (
                    <div key={cat} className="bg-gray-50 rounded p-2 text-xs">
            ...
            <div className="mt-3">
              <h4 className="text-xs font-medium text-gray-500 mb-2">Per-Entry Results</h4>
              <div className="space-y-1 max-h-96 overflow-auto">
                {results.map((r) => (
                  <details key={r.id} className="border border-gray-200 rounded text-xs">
                    <summary className="px-3 py-1.5 cursor-pointer bg-gray-50">
