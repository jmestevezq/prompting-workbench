...
function TemplateTab({ agent }: { agent: Agent | null }) {
  ...
  if (loading) {
    return <div className="p-6 text-slate-400 text-sm">Loading template...</div>
  }
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl">
      ...
      {template.raw_template && (
        <section>
          <h3 className="text-sm font-semibold text-slate-700 mb-2">Raw Template (.ftl)</h3>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="text-left px-3 py-2 font-medium text-slate-600">Value Preview</th>
                </tr>
              </thead>
              <tbody>
                {varDefs.map((def: Record<string, unknown>, i: number) => {
                  ...
                  return (
                    <tr key={i} className="border-t border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2 text-slate-600 truncate max-w-xs">{preview}</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      {/* Rendered prompt */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 mb-2">Rendered System Prompt</h3>
            ...
            {template.tool_details.map((tool, i) => (
              <div key={i} className="border border-slate-200 rounded p-3">
                ...
                {tool.usageGuidelines && (
                  <div className="text-xs text-slate-500 mt-1">{tool.usageGuidelines}</div>
                )}
              </div>
            ))}
          </div>
      ...
      {template.widget_details && template.widget_details.length > 0 && (
        <section>
          <h3 className="text-sm font-semibold text-slate-700 mb-2">Widgets</h3>
          <div className="space-y-2">
            {template.widget_details.map((w, i) => (
              <div key={i} className="border border-slate-200 rounded p-3">
                ...
                {w.example && (
                  <pre className="text-xs bg-slate-50 p-2 mt-1 rounded overflow-auto">
                    {w.example}
                  </pre>
                )}
              </div>
...
function VersionsTab({
  ...
  return (
    <div className="h-full flex">
      {/* Left — version list */}
      <div className="w-[480px] border-r border-slate-200 flex flex-col bg-white">
      ...
      <div className="flex-1 p-6 overflow-auto">
        {showCreate ? (
          <div className="max-w-2xl space-y-4">
            <h3 className="text-sm font-semibold text-slate-700">Create New Version</h3>
            <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-1.5 text-sm font-medium bg-slate-200 text-slate-700 rounded hover:bg-slate-300"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="max-w-3xl space-y-4">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-semibold text-slate-800">{selected.version_label}</h3>
            ...
            {selected.widget_details && selected.widget_details.length > 0 && (
              <section>
                <h4 className="text-sm font-medium text-slate-700 mb-1">Widgets</h4>
                <div className="space-y-1">
                  {selected.widget_details.map((w, i) => (
                    <div key={i} className="border border-slate-200 rounded p-2 text-sm">
                      <span className="font-medium">{w.name}</span>
                      <span className="text-slate-500 ml-2">— {w.description}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {selected.id !== agent.active_version_id && (
              <button
                ...
                className="px-4 py-1.5 text-sm font-medium bg-emerald-600 text-white rounded hover:bg-emerald-700"
              >
                Set as Active
              </button>
            )}
          ...
          <div className="text-slate-400 text-sm">Select a version to view details</div>
        )}
      </div>
    </div>
  )
